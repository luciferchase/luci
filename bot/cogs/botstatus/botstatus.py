...
class Botstatus(commands.Cog):
    ...
    @competing.command(name="idle")
    async def c_idle(self, ctx, *, text: str):
        if len(text) > 128:
            await ctx.send(_("The chracter limit for status messages is 128."))
        else:
            await self.config.status.set(("competing", "idle", text))
            await self.setfunc("competing", "idle", text)
            await ctx.send(_("Status set to ``Idle | Competing {text}``").format(text=text))
